...
$.fn.share = function(options) {
    ...
    if(!$(this).hasClass('share_with_status') ){
        ...
        pic = $.map($('.img-box'), function(item, index){return origin + $(item).children('img').attr('src')}).join('||')
    }
        ...
        var param = {
            ...
            count:'',
            appkey:'1153655536',
            title:'我发现了一篇很实用的#'+ $('.share-note-type').text().trim()+'#：' + $(".share-title").text().trim()+ '［'+ $('.share-note-day-count').text().trim() + '天/' +$('.img-box').length+ '图］，并分享给大家！'   ,
            pic: pic,
            ralateUid: '3080252823',
            language:'zh_cn',
